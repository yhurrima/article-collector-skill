...
- 选项 2：”没有 api，用我正在使用的agent的原生能力”。如果用户选择没有 API，浏览器插件先把链接保存到飞书表格，状态为”待处理”；用户发送链接时，Agent 直接抓取并总结。
...
同时告诉用户：
- 强烈推荐安装浏览器插件，使用体验更好；安装后浏览网页时点击插件即可收藏当前文章。
- 询问用户是否要安装浏览器插件。
- 如果用户确认要安装浏览器插件，Agent 应协助用户使用当前项目内的 `chrome-extension/` 目录，并告诉用户通过 Chrome 开发者模式加载未打包的程序：
  1. 打开 Chrome，进入 `chrome://extensions/`。
  2. 打开右上角“开发者模式”。
  3. 点击“加载未打包的程序”。
  4. 选择当前项目下的 `chrome-extension/` 文件夹。
  5. 确认扩展出现后，打开任意文章页面，点击插件即可收藏。
- 如果不安装浏览器插件，也可以直接在 Agent 的对话框里发送“收藏这篇文章，这是文章链接：<url>”，Agent 会按相同流程处理。
...
IM 推送命令（从 config.py 读取 FEISHU_IM_CHAT_ID）：
```bash
lark-cli im +messages-send --as bot --chat-id $FEISHU_IM_CHAT_ID --text "已收藏: {标题}\n{摘要}"
```
...
### /reading-stats
当用户说“我想看我的阅读统计”、“打开阅读统计”或“查看阅读仪表盘”时，创建或复用名为“阅读统计”的飞书多维表格仪表盘，并把仪表盘链接发给用户。
...
- 包含分类饼图：按 `分类` 统计文章数量。
- 包含来源柱状图：按 `来源` 统计文章数量，并按数量降序展示。
- 包含每天收藏文章数量折线图：按 `保存日期` 统计每日收藏数量，并按日期升序展示。

处理规则：
- 如果“阅读统计”仪表盘已存在，复用现有仪表盘，不重复创建。
- 如果缺少图表组件，只补齐缺失的“分类分布”、“来源分布”、“每日收藏趋势”组件。
