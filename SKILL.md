...
- **未配置（新用户）**：直接帮用户创建一个飞书多维表格，创建文章收藏表所需字段，然后将 `FEISHU_BASE_APP_TOKEN`、`FEISHU_ARTICLES_TABLE_ID`、`FEISHU_WEEKLY_TABLE_ID`、`FEISHU_IM_USER_ID`（或 `FEISHU_IM_CHAT_ID`）写入 `backend/.env`。创建完成后告诉用户："我会创建一个飞书多维表格，以后你收藏的文章都可以在这里查看：<飞书多维表格链接>"
...
如果用户选择安装，Agent 只需引导用户加载浏览器插件（队列服务已在定时任务安装时自动配置为开机启动）：
...
如果用户选择安装插件但尚未配置定时任务（例如选择了”不定时”发送），Agent 需要运行 `python3 backend/install_scheduler.py` 确保队列服务已安装。
...
IM 推送命令（从 config.py 读取 `FEISHU_IM_USER_ID` 或 `FEISHU_IM_CHAT_ID`），使用 `--markdown` 格式以支持换行和富文本。优先使用 `--user-id` 私信推送，其次使用 `--chat-id` 群聊推送。

**重要：`--markdown` 参数中不能使用 `\n` 表示换行，`\n` 会被当作字面量。必须使用 heredoc 传递多行内容：**
```bash
# 私信推送（优先）
lark-cli im +messages-send --as bot --user-id $FEISHU_IM_USER_ID --markdown "$(cat <<'MARKDOWN'
**已收藏: {标题}**
...
# 优先私信，其次群聊
if sys.argv[1]:
    subprocess.run(['lark-cli', 'im', '+messages-send', '--as', 'bot', '--user-id', sys.argv[1], '--markdown', msg], check=True)
elif sys.argv[2]:
    subprocess.run(['lark-cli', 'im', '+messages-send', '--as', 'bot', '--chat-id', sys.argv[2], '--markdown', msg], check=True)
...
- FEISHU_IM_USER_ID — 机器人私信推送的用户 ID（优先）
- FEISHU_IM_CHAT_ID — 机器人推送的群聊 ID（备选）
...
**队列服务自动启动**
运行 `python3 backend/install_scheduler.py` 时会自动安装队列服务为 launchd 后台服务，开机自动启动，无需手动运行 `./run.sh`。

使用方式：
1. Chrome 开发者模式加载 chrome-extension/ 文件夹
2. 点击插件 → 自动将当前页面 URL 发送到 `http://127.0.0.1:5679/queue`
3. 后端自动抓取、调用 API 摘要、写入飞书表格并发送确认消息
